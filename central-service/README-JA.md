...
## 負荷特性(実測)
`python3 ../ci/load_test.py`(モックなし、実際のStore・実際のHTTPハンドラー)で計測。数値は実行環境のCPU性能に依存するため絶対値ではなく傾向として読んでください。GitHub Actions ubuntu-24.04ランナーでの実測例:

- 同時接続1〜16では毎秒約1,200〜1,350リクエストまでほぼ線形に伸びる
- 同時接続64では毎秒約1,000リクエストへ**低下**し、p95レイテンシが急増、接続エラーが発生し始める（`service.py`は`http.server.HTTPServer`で単一処理・単一接続直列処理のため、これが実質的な上限）
- SQLiteの書き込み(`BEGIN IMMEDIATE`)自体は同時64件でも毎秒600件超を処理でき、ロック待ちタイムアウトでの失敗は0件。つまり**ボトルネックはSQLiteではなくHTTPサーバーの単一処理設計**
- 詳細な生データは `reports/load-test.json` に出力される

このことから、次に手を入れるべきはPostgreSQL移行より先に**HTTP層の並行処理化**(`ThreadingHTTPServer`への切替、または本物のWSGIサーバー配置)である可能性が高いです。ただし切替後はSQLiteの`BEGIN IMMEDIATE`書き込みロックが新しい上限になる見込みで、その先の規模になって初めてPostgreSQL移行の検討価値が出てきます。

## 商用化の次工程
価格・課金対象・締日・遅延受付期限の確定、期間の締め処理、訂正・返金、中央の登録承認、契約/支払いの署名付きWebhook、購入/解約用ポータル、支出上限、滞納処理、PostgreSQL等の永続DBとワーカー。
