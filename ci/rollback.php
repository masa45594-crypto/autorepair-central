...
try {
 ...
 add_filter('home_url',function($url){return 'http://127.0.0.1:8080/aaihb-rollback-health.php';},10,1);
 ...
 $restored=strpos($source,'aaihb_rollback_fixture_loaded')!==false && strpos($source,"'v1'")!==false && strpos($source,'aaihb_broken_fixture')===false;$history=isset($last['id'],$last['plugin'],$last['message']) && $last['plugin']===$plugin && strpos($last['message'],'戻しました')!==false;$database=get_option('aaihb_rollback_db_marker')==='before-update';
